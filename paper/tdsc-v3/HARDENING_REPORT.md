# TDSC v3 Hardening Report

## Baseline State

- Date: 2026-07-07T14:30:06+08:00
- Branch: `tdsc-v3-hardening`
- Commit: `c95e6eda73217a4565cb0add0abfd92a168b26ee`
- Docker Compose: `Docker Compose version v2.40.3-desktop.1`
- Python: `Python 3.13.5`
- PostgreSQL: `PostgreSQL 16.14 (Debian 16.14-1.pgdg13+1)`
- Current TDSC manuscript path: `paper/tdsc-v2/sessionbound-tdsc.tex`
- Current TDSC PDF path: `paper/tdsc-v2/sessionbound-tdsc.pdf` (541K)

Recent commits:

```text
c95e6ed 1
3479093 1
ca195e5 1
98a47eb Extend TDSC v2 credential token tests
0792612 Add TDSC v2 baseline experiments
5dce85f Fix TDSC draft layout issues
e000cac Add TDSC journal draft
75ef952 Add hosted demo link to README
```

Initial `git status --short --branch`:

```text
## tdsc-v3-hardening
```

## Docker Startup

Commands run from `/home/wmm/taskbound`:

```bash
docker compose down -v || true
docker compose up -d --build api
docker compose ps
sleep 10
curl -sS -o /dev/null -w "%{http_code}\n" http://localhost:8000/
curl -sS -o /dev/null -w "%{http_code}\n" http://localhost:8000/docs
```

Startup result:

```text
taskbound-api-1        taskbound-api   Up, 0.0.0.0:8000->8000/tcp
taskbounddb-postgres   postgres:16     Up, healthy
GET /                  200
GET /docs              200
```

PostgreSQL version:

```text
PostgreSQL 16.14 (Debian 16.14-1.pgdg13+1) on x86_64-pc-linux-gnu, compiled by gcc (Debian 14.2.0-19) 14.2.0, 64-bit
```

## Notes

- The requested instruction references `paper/tdsc-v1/`, but the current
  repository contains the active TDSC manuscript in `paper/tdsc-v2/`.
- This hardening sprint therefore records v3 outputs under `paper/tdsc-v3/`
  and updates the current TDSC manuscript in `paper/tdsc-v2/`.

## Experiment Results

AST validation:

- Script: `paper/tdsc-v3/scripts/ast_validation_eval.py`
- Latest raw result: `paper/tdsc-v3/raw_results/ast_validation_20260707_065023.json`
- Result: 17 / 17 cases passed
- Status: implemented as API-layer AST preflight before `taskbound.run(...)`

Adversarial SQL:

- Script: `paper/tdsc-v3/scripts/adversarial_sql_eval.py`
- Latest raw result: `paper/tdsc-v3/raw_results/adversarial_sql_20260707_145022.json`
- Result: 28 / 28 expected classifications passed
- Classification counts: 22 blocked, 5 allowed but accounted, 1 known limitation

Overhead breakdown:

- Script: `paper/tdsc-v3/scripts/overhead_breakdown.py`
- Latest raw JSON: `paper/tdsc-v3/raw_results/overhead_breakdown_20260707_064236.json`
- Latest raw CSV: `paper/tdsc-v3/raw_results/overhead_breakdown_20260707_064236.csv`
- Result: 0 errors across supported modes
- Main finding: safe-view/session-claim evaluation dominates the measured
  small-dataset overhead; receipts and budget updates do not dominate.

Canonical validation:

- Command: `python3 scripts/sessionbound_agent_eval.py --base-url http://localhost:8000`
- Result: 24 / 24 scenarios passed
- Output: `eval_runs/sessionbound_agent_eval_1783407368.json` and `.md`
  generated by the existing script; these are not part of the v3 committed
  artifact set.
