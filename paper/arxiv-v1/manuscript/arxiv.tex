...
\hypertarget{sessionbound-turning-enterprise-task-approval-into-budgeted-database-sessions}{%
\section{SessionBound: Turning Enterprise Task Approval into Budgeted
Database
Sessions}\label{sessionbound-turning-enterprise-task-approval-into-budgeted-database-sessions}}

\hypertarget{safe-views-budgets-and-receipts-for-open-ended-enterprise-analysis}{%
\subsection{Safe Views, Budgets, and Receipts for Open-Ended Enterprise
Analysis}\label{safe-views-budgets-and-receipts-for-open-ended-enterprise-analysis}}
...
\textbf{Code availability.} The prototype source code and synthetic
...
\subsection{2. Motivation: Enterprise Tasks Are Approved Above the
...
\subsubsection{2.1 SaaS is strong at fixed workflows and weak at
temporary
...
\subsubsection{2.2 Enterprise approval is not database policy
...
\subsubsection{2.3 Running example: travel reimbursement anomaly
...
\subsubsection{3.1 Trusted components}\label{trusted-components}}
...
\subsubsection{3.2 Untrusted or partially trusted
...
\subsubsection{3.3 In-scope threats}\label{in-scope-threats}}
...
\subsubsection{3.4 Out of scope}\label{out-of-scope}}
...
\subsection{4. SessionBound Model}\label{sessionbound-model}}
...
\subsubsection{4.1 Task template}\label{task-template}}
...
\subsubsection{4.2 Task application}\label{task-application}}
...
\subsubsection{4.3 Task approval}\label{task-approval}}
...
\subsubsection{4.4 Task token}\label{task-token}}
...
\subsubsection{4.5 Budgeted database
...
\subsection{5. Control Plane: Templates, Applications, Approvals, and
...
\subsubsection{5.1 Business users approve tasks, not
...
\subsubsection{5.3 Budget assignment}\label{budget-assignment}}
...
\subsection{6. SessionBoundDB Runtime}\label{sessionbounddb-runtime}}

SessionBoundDB is the PostgreSQL-based database runtime for
SessionBound.

\hypertarget{session-binding-and-credential-token-binding}{%
\subsubsection{6.1 Session binding and credential-token
...
\subsubsection{6.2 Safe views as the query
...
\subsubsection{6.3 Operation policy}\label{operation-policy}}
...
\subsubsection{6.4 Query decision
...
\subsection{7. Anti-Evasion Scope}\label{anti-evasion-scope}}
...
\subsubsection{7.1 Direct boundary
...
\subsubsection{7.2 Inference and side
...
\subsubsection{7.3 Adversarial SQL
...
\subsection{8. Budget Semantics}\label{budget-semantics}}
...
\subsubsection{8.1 Budget vector}\label{budget-vector}}
...
\subsubsection{8.2 Budget dimensions}\label{budget-dimensions}}
...
\subsubsection{8.3 Budget limitations}\label{budget-limitations}}
...
\subsection{9. Schema Drift and Safe View
...
\subsubsection{9.1 Binding model}\label{binding-model}}
...
\subsubsection{9.2 Drift behavior}\label{drift-behavior}}
...
\subsection{10. Receipts}\label{receipts}}
...
\subsection{11. Prototype Implementation and Production
Hardening}\label{prototype-implementation-and-production-hardening}}

\hypertarget{prototype}{%
\subsubsection{11.1 Prototype}\label{prototype}}
...
\subsubsection{11.3 Query example}\label{query-example}}
...
\subsection{12. Evaluation}\label{evaluation}}
...
\subsubsection{12.1 Functional scenario
suite}\label{functional-scenario-suite}}

We evaluated the imported PostgreSQL prototype after the public
SessionBound rename at commit \texttt{f793787} using Docker Compose,
PostgreSQL 16.14, and the FastAPI task-token and dynamic-credential API.
The detailed validation run passed 18 of 18 tested scenarios. The core
allowed query patterns worked, and direct sensitive-field, raw-schema,
mutation, DDL, query-budget, unique-row disclosure-budget, and
payload-aggregation violations were denied.
...
Other month & scope escape & Transparent scope filtering & Transparent
scope filtering \\
...
\texttt{array\_agg(e.expense\_id)} payload & aggregation payload &
Denied & Denied \\
\texttt{string\_agg(employee\_name,\ \textquotesingle{},\textquotesingle{})}
payload & aggregation payload & Denied & Denied \\
...
\subsubsection{12.2 Performance
...
We ran a microbenchmark at commit \texttt{cb2d4ca} with 10 warmup
iterations and 100 measured iterations per query pattern. The baseline
is an admin/test role executing equivalent SQL over raw
...
\subsubsection{12.3 Threat-to-defense
...
\subsubsection{13.1 OAuth, agent identity, and authenticated
...
\subsubsection{13.2 PAuth and task-scoped service
...
\subsubsection{13.3 Data Product MCP and enterprise data product
...
\subsubsection{13.4 Oracle DDS and database-enforced access
...
\subsubsection{13.5 Zanzibar and relationship-based
...
\subsubsection{13.6 Differential privacy, inference control, query
auditing, and
...
\subsection{14. Future Work}\label{future-work}}
