...
%
% TDSC journal-preparation draft derived from paper/arxiv-v1.
% Keep this tree independent from the public arXiv source.
...
\makeatletter
\renewcommand{\@cite}[2]{\unskip\raisebox{0.75ex}{\scriptsize[#1\if@tempswa , #2\fi]}}
\makeatother
...
the approved boundary. For a security-oriented journal version, the
paper treats this principle as a database-enforced authorization
boundary for enterprise AI agents rather than as an agent-planning or
multi-agent coordination contribution.
...
attacks. The TDSC journal version should extend this section with
larger-scale benchmark data and baseline comparisons against
raw PostgreSQL, role-only access, row-level-security-only access,
safe-view-only access, and the full SessionBound runtime.
...
\hypertarget{baseline-plan-for-the-journal-version}{%
\subsection{Baseline plan for the journal
version}\label{baseline-plan-for-the-journal-version}}

The current draft reports a raw PostgreSQL versus SessionBound
microbenchmark. A complete journal submission should separate the
security and performance contribution across at least five baselines:
...
These baselines should be evaluated on the same task templates,
synthetic datasets, SQL scenario suite, and benchmark environment so
that the paper can distinguish access-surface reduction from
task-bound session enforcement.
