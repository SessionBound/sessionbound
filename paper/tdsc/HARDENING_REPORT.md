...
- Latest raw result: generated by the final revision of
  `paper/tdsc/scripts/rollback_audit_eval.py`
- Result: 4 / 4 cases passed.
- Status: parser/hook, wrapper, and executor denials emit autonomous receipts;
  no test-side `fail_receipt` call is used, and native rollback checks include
  zero prefix release.
...
- Main finding: raw/role/safe-view modes are sub-millisecond, the
  RLS+Safe View+Short Credential+Audit baseline is 1.058--1.313 ms p50, and
  the full SessionBound wrapper reference path is 128.4--136.1 ms p50.
