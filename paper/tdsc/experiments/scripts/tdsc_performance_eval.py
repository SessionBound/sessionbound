...
PATTERNS = {
    ...
    "GROUP BY": {
        "safe": "SELECT department_name, count(*) AS n, sum(amount) AS total FROM {schema}.expenses GROUP BY department_name ORDER BY total DESC",
        "raw": "SELECT d.department_name, count(*) AS n, sum(e.amount) AS total FROM app_data.expenses e JOIN app_data.departments d ON e.department_id=d.department_id WHERE e.tenant_id='company_a' AND e.expense_month='2026-06' AND e.department_id='dep_sales' GROUP BY d.department_name ORDER BY total DESC",
        "sb": "SELECT department_name, count(*) AS n, sum(amount) AS total FROM expenses GROUP BY department_name ORDER BY total DESC",
    },
    "CTE": {
        "safe": "WITH high AS (SELECT expense_id, amount FROM {schema}.expenses WHERE amount > 1000) SELECT count(*) AS high_count FROM high",
        "raw": "WITH high AS (SELECT expense_id, amount FROM app_data.expenses WHERE tenant_id='company_a' AND expense_month='2026-06' AND department_id='dep_sales' AND amount > 1000) SELECT count(*) AS high_count FROM high",
        "sb": "WITH high AS (SELECT expense_id, amount FROM expenses WHERE amount > 1000) SELECT count(*) AS high_count FROM high",
    },
    "Window": {
...
def run_once(cur, baseline: Baseline, pattern: dict[str, str]) -> int:
    if baseline.mode == "sessionbound":
        ...
        cur.execute(sql_for(baseline, pattern))
