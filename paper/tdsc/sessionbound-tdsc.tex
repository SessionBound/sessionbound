...
\texttt{unique\_entities} & New distinct \texttt{expense\_id} values in released detail tuples for the same \texttt{budget\_account} are charged once. Repeated disclosure of the same ID does not consume additional entity budget, but each query still consumes \texttt{query\_count} and \texttt{result\_rows} budgets, as well as any applicable bytes or column-weight budgets. \\
...
then checks group and disclosure policy before returning rows. High-value
