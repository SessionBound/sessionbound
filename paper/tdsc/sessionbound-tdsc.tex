...
Evaluation validates 24 of 24 canonical scenarios and a 34-case
adversarial SQL suite with 27 blocked cases, 7 allowed safe-view
analytical cases, and no remaining direct small-group aggregate release
in the tested suite. In the wrapper-based reference path, full
SessionBound has p50 latency of 17.4--18.6 ms across representative
query patterns on the default seed. The same wrapper path reaches
6.17--6.37 s at 100k scoped rows, exposing a materialization/accounting
bottleneck. A hook-only parse/analyze guard microbenchmark shows
0.125--0.138 ms p50 structural-check latency, indicating that structural
guarding is not the dominant bottleneck; native executor-accounting
...
\item
  ...
  individual query, as the execution unit that should bind agentic SQL.
\item
...
The latest revision run uses an adversarial SQL suite with 34 cases across
...
classifications: 27 cases were blocked and 7 safe-view analytical cases
were allowed and accounted for. Direct small-group aggregate release is
blocked by the configured minimum-group policy in the tested suite.
...
Small-group aggregate inference & direct entity grouping, \texttt{HAVING} probe, filtered small group, high configured \(k\) & 5 blocked, 2 allowed & direct small-group aggregate release denied; large verified aggregate allowed \\
...
the tens of milliseconds, while full SessionBound reaches 6.17--6.37 s.
...
safe-view-only execution stays below 83 ms for the aggregate and below
31 ms for top-k. Full SessionBound reaches 6.17--6.37 s at 100k rows.
These results expose the scale-sensitive
...
measurement. Minimum-group policy mitigates direct small-group aggregate
release, but arbitrary semantic inference across multiple allowed
answers remains out of scope. The budget vector is operational and does
...
\caption{Synthetic scale sweep p50 latency in milliseconds. Safe denotes safe-view-only execution; RLS+SV+Audit is the strong RLS baseline; SB denotes the wrapper reference path with full SessionBound accounting. Native executor-accounting scale was not measured.}
\label{tab:scale-sweep}
\begin{tabular}{@{}llrrrr@{}}
\toprule
Rows & Query & Raw & Safe & RLS+SV+Audit & SB \\
\midrule
1k & Agg. & 0.86 & 1.02 & 2.00 & 79.03 \\
1k & Top-k & 0.43 & 0.64 & 1.46 & 72.63 \\
10k & Agg. & 5.39 & 6.60 & 7.49 & 618.95 \\
10k & Top-k & 2.02 & 2.86 & 3.65 & 660.25 \\
100k & Agg. & 68.81 & 82.31 & 74.87 & 6365.20 \\
100k & Top-k & 13.82 & 30.70 & 19.93 & 6169.55 \\
\bottomrule
\end{tabular}
\end{table}
...
Future work should move in five directions: cross-database and lakehouse
federation that preserves one approval, budget, and receipt chain;
planner- or executor-level integration for production SQL enforcement
and business-object accounting; richer aggregate leakage controls,
multi-query reconstruction-risk checks, and optional DP when statistical
privacy is required; stronger receipt anchoring through append-only
logs or external notarization; and safe-view authoring workflows with
linting, hashing, adversarial tests, and migration-driven token
invalidation.
...
SessionBound is a research prototype and reference architecture with
these limitations:

\begin{itemize}
\tightlist
\item
  SQL validation includes AST preflight and a PostgreSQL hook/executor
  prototype, but not a production-grade formal SQL safety proof;
\item
...
\item
  ...
  transparent scope filtering; high-risk payload aggregation functions
  are conservatively blocked rather than sensitivity-scored;
\item
...
\item
  ...
  ablations, and a 1k/10k/100k wrapper-reference scale sweep, but not a
  broad native executor-accounting workload study;
\item
  ...
  audit retention, lifecycle cleanup, credential brokerage, and key
  management remain production infrastructure outside this demo;
\item
...
default dataset, the accounting-complete wrapper path runs in the
17--19 ms p50 range, while raw, role-only, and safe-view-only baselines
remain sub-millisecond and the stronger RLS+safe-view+audit baseline is
near 1 ms. At 100k scoped rows, the PL/pgSQL reference runtime becomes
multi-second while raw PostgreSQL, safe-view-only execution, and the RLS
baseline remain much faster. The hook-only microbenchmark shows that
structural parse/analyze guarding is not the multi-second bottleneck.
This does not change the core task-bound session model, but it does
define the production engineering path: move from the wrapper-based
reference runtime toward measured native executor accounting and
optimized PostgreSQL integration. The contribution is the task-bound
session contract, not a perfect SQL safety proof.
