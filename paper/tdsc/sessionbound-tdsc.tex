...
In the current prototype, a direct database connection does not mean
bare \texttt{SELECT} privilege on safe views. The generated runtime
credential may bind one task and call \texttt{taskbound.run},
\texttt{taskbound.command}, and receipt/state inspection functions, but
it is not granted \texttt{SELECT} on either raw application tables or the
safe-view schema. Thus a direct PostgreSQL connection can bypass the HTTP
API, but it cannot bypass the runtime accounting path: approved SQL over
safe views must still be submitted through the SDK's \texttt{query(sql)}
method, which wraps \texttt{taskbound.run}; query budgets, disclosure
budgets, and receipts are applied there. A production deployment that
chooses to expose native bare \texttt{SELECT} syntax to agents must move
this same accounting contract into always-on planner/executor hooks
before any rows are released to the client.
...
\texttt{taskbound.run(sql)} is the database accounting entrypoint for
read-only analytical SQL. The agent-facing SDK exposes this as a
native-feeling \texttt{query(sql)} method: the agent supplies ordinary SQL
over approved safe-view names, and the SDK submits that SQL as a
parameter to \texttt{taskbound.run}. High-value writes should use
controlled commands or stored procedures. The runtime rejects mutations,
DDL, unsafe utility commands, raw schema access, denied fields, and
blocked schemas.
...
grants. Agents call the SDK's \texttt{query(sql)} method; the SDK calls
\texttt{taskbound.run(sql)} with the supplied SQL as a parameter. The
runtime role cannot access \texttt{app\_data} directly. In this
prototype, agents also do not receive bare \texttt{SELECT} grants on the
safe views; safe views are the logical query surface, but
\texttt{taskbound.run} is the database accounting entrypoint. A bare
direct query such as
\texttt{SELECT ... FROM taskbound.expenses} is denied by PostgreSQL
privilege checks rather than being allowed outside the receipt chain.
...
We separately evaluated the native-feeling SDK query surface. The SDK
suite passed 2 of 2 cases: \texttt{TaskboundSession.query(sql)} returned
two safe-view rows, incremented the task query count to one, and produced
an allow receipt with two returned rows; a bare
\texttt{SELECT ... FROM taskbound.expenses} in the same bound session was
blocked by PostgreSQL privileges. This establishes the intended user
experience boundary: agents can write ordinary analytical SQL through
\texttt{query(sql)}, while bypassing the SDK does not bypass the
accounting path.

We also evaluated the database hook path directly. The
\texttt{sessionbound\_guard} suite passed 11 of 11 cases. It confirmed
that ordinary agent roles cannot enable trusted guard GUCs, that an
approved query over safe views succeeds when a direct database credential
calls \texttt{taskbound.run}, and that \texttt{UNION} stacking, catalog
access, recursive CTEs, direct runtime-helper abuse, non-\texttt{SELECT}
SQL, raw schema access, payload aggregation, and unapproved safe-view
OIDs are blocked by the database path. It also confirms that a bare
safe-view \texttt{SELECT} by the generated credential is blocked by
database privileges. The table's ``direct database''
wording means that the experiment bypasses the HTTP API and connects to
PostgreSQL as the generated runtime credential; it does not mean that the
credential is granted bare \texttt{SELECT} over safe views.
...
API query endpoint and connect to PostgreSQL directly as the generated
runtime credential; approved SQL is still submitted through
\texttt{taskbound.run}, because the runtime credential has no bare
\texttt{SELECT} grant on safe views.}
...
Safe-view query through runtime & Agent credential direct DB connection calling \texttt{taskbound.run} & 1 allowed / 1 & Approved safe-view relation OIDs accepted; runtime produces budget/accounting receipt \\
Bare safe-view query & Agent credential direct DB session without \texttt{taskbound.run} & 1 blocked / 1 & No bare safe-view \texttt{SELECT} grant; cannot bypass accounting path \\
Set operation, catalog, recursive CTE & Agent credential direct DB connection calling \texttt{taskbound.run} & 3 blocked / 3 & Denied by parser/analyzer hook \\
Runtime helper abuse & Agent credential direct DB connection calling \texttt{taskbound.run} & 1 blocked / 1 & Runtime wrapper denies direct helper calls \\
