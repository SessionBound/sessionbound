...
\item
  ...
  versioning, exposed-column hashing, disclosure budgets,
  aggregate/window shape policy, and receipts are bound into one session
  object.
\item
...
filtered aggregate release, and window release. Denied aggregate/window
queries emit denial receipts and do not release disclosure rows.
...
\midrule
\texttt{query\_count} & Wrapper and native paths: one debit inside the fenced release-barrier transition for an allowed execution. Preflight and policy denials emit denial receipts without result/entity debit in the current prototype. \\
...
preflight receipt writes and unsupported syntax rejected before
parse/analyze use an approved-name fallback. This field is hashed audit
context rather than the enforcement predicate; relation enforcement still
comes from AST and safe-view OID checks.
...
\texttt{expense\_id}-only projection heuristic is used. For allowed
releases, receipt and budget updates occur in one fenced release-barrier
transition on the autonomous same-database audit channel. Parser, wrapper,
hook, and API denials use the same audit channel for rollback-surviving
denial receipts.
...
\texttt{sessionbound\_guard} path directly and passed 18 of 18
cases in \texttt{sessionbound\_guard\_hook\_20260719\_162553.json}. It
...
shapes, direct aggregate/window release, and \texttt{EXPLAIN ANALYZE}.

A separate path-consistency run,
\texttt{path\_consistency\_20260719\_184242.json}, passed 12 of 12
cases across API, direct-wrapper, and direct-native paths. It compares
36 path observations for allowed detail projection, join, non-aggregate
CTE, denied-column, raw-schema, catalog, set-operation,
aggregate/window, side-effect-function, and \texttt{TABLESAMPLE}
...
no DB drift &
crypto delegation \\
Qapla/Blockaid/Sieve~\cite{qapla2017,zhang2022blockaid,pappachan2020sieve} &
user/context/policy &
adapter/proxy/MW &
app/user identity &
rewrite/guard &
...
no task snapshot &
policy compliance \\
Agent policies~\cite{shi2025progent,wang2025agentspec,jia2025taskshield,sharma2026pauth} &
task/tool/op &
runtime/gateway &
no DB login &
tools/services &
