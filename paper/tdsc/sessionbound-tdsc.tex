...
We present SessionBound, a control-plane and database-runtime
architecture that turns approved enterprise tasks into budgeted
...
runtime, SessionBoundDB, binds an approved task token to a
short-lived agent session and enforces safe views, row scope, denied
fields, operation limits, structural SQL policy, query budgets,
disclosure budgets, and query receipts at SQL execution time. The
hardening prototype includes both API-layer AST validation and an
experimental PostgreSQL hook path for database-resident structural SQL
enforcement.

The current evaluation validates 24 of 24 canonical scenarios and a
28-case adversarial SQL suite with 22 blocked cases, 5 allowed
safe-view analytical cases, and 1 known limitation. Performance results
separate the security model from the reference runtime. On the default
synthetic seed, safe-view-only p50 latency is 13.3--18.1 ms and full
SessionBound p50 latency is 14.6--18.6 ms, indicating that small-scale
overhead is dominated by the safe-view/session runtime rather than by
receipt-chain insertion or budget updates. A 100k-row synthetic scale
sweep exposes multi-second latency in the current PL/pgSQL wrapper path,
motivating a parser-, planner-, or executor-hook implementation for
...
Code availability. The prototype source code, synthetic
evaluation dataset, validation reports, benchmark outputs, and LaTeX
source for the paper are available at
\url{https://github.com/SessionBound/sessionbound}.
\par\noindent Preprint availability. The arXiv version is available at
...
\item
  PostgreSQL prototype and hardening analysis. We implement
  SessionBoundDB as a PostgreSQL-based runtime for SessionBound and
  demonstrate it on enterprise travel reimbursement analysis. The
  hardening prototype includes a \texttt{sessionbound\_guard} C extension
  loaded through \texttt{shared\_preload\_libraries} that uses
  PostgreSQL's \texttt{post\_parse\_analyze\_hook} to reject unsafe SQL
  structures before dynamic execution. We also analyze anti-evasion
  scope, budget semantics, schema drift, credential-token binding,
  receipt structure, and prototype-to-production hardening requirements.
\end{enumerate}
...
A fixed endpoint such as \texttt{get\_expense\_summary}, parameterized
by department and month, is too narrow. A broad database account is too
dangerous. A prompt instruction such as ``do not access salary'' is not
a security mechanism. SessionBound's answer is to approve a task and
bind it to a database session with safe views, scope, budgets, and
receipts.
...
The phrase disclosure budget can be misleading if interpreted
...
\item
  per-query payload bytes: prevents one-row JSON, array, XML,
  or string payloads from smuggling a large relation as a single tuple;
\item
  aggregate payload shape: optionally blocks or heavily charges
  high-risk aggregation functions such as \texttt{json\_agg},
  \texttt{array\_agg}, \texttt{string\_agg}, and \texttt{xmlagg};
\item
  unique business entities: tracks exposure of objects such as
  expenses, employees, tickets, or customers;
\item
  column weights: charges more for more sensitive exposed
  attributes;
\item
  detail versus aggregate multipliers: charges detail rows more
  than coarse aggregates;
\item
  small-group penalties: charges more when aggregates describe
  very small groups.
\end{itemize}
...
conservative database-side checks, a database-resident PostgreSQL hook
path, and PostgreSQL permissions. Agents do not receive raw schema
grants. Agents call \texttt{taskbound.run(sql)}. The runtime role cannot
access \texttt{app\_data} directly. Safe views are the query surface.
...
\texttt{post\_parse\_analyze\_hook} and reads trusted SUSET GUCs populated
by \texttt{taskbound.bind\_task}, including the bound task state and the
approved safe-view OIDs from the runtime registry. Before dynamic
execution, \texttt{taskbound.run} calls
\texttt{public.sessionbound\_guard\_check(sql)}, which uses
\texttt{SPI\_prepare} to trigger PostgreSQL parse/analyze on the exact SQL
text. If the hook denies the query, the runtime emits a denial receipt and
...
query-shape cases, and the PostgreSQL hook evaluation passed 10 of 10
...
AST preflight, \texttt{taskbound.run}, budget state, and receipts. The
hook evaluation bypasses the API query endpoint for agent cases by using
generated runtime credentials directly against PostgreSQL, and it uses
trusted superuser setup for hook-only structural checks. The overhead
breakdown uses direct PostgreSQL connections from the Compose network so
that it isolates database runtime overhead from HTTP, model calls,
dynamic credential creation, and API-layer AST parsing.
...
scenarios. Table~\ref{tab:validation-scenarios} lists the full canonical
suite and includes the corresponding scenario identifiers used by
\texttt{scripts/sessionbound\_agent\_eval.py}.
...
\texttt{sessionbound\_guard} suite passed 10 of 10 cases. It confirmed
that ordinary agent roles cannot enable trusted guard GUCs, that an
approved safe-view \texttt{SELECT} succeeds through a direct database
credential call, and that \texttt{UNION} stacking, catalog access,
recursive CTEs, direct runtime-helper abuse, non-\texttt{SELECT} SQL, raw
schema access, payload aggregation, and unapproved safe-view OIDs are
blocked by the database path.

\begin{table*}[!t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{PostgreSQL hook enforcement evaluation. Agent cases bypass the
API query endpoint and call PostgreSQL directly as the generated runtime
credential.}
...
Within the measured database path, the overhead comes from several
implementation layers in the reference prototype. First,
...
The scale results in Table~\ref{tab:scale-sweep} show a different
...
parser/analyzer, planner, or executor hooks.
...
limitation of the current PL/pgSQL reference runtime and motivate the
production hook path discussed above.
...
path is an experimental parse/analyze guard invoked before dynamic
execution; it is not yet production-grade planner or executor
enforcement. Small-group aggregate inference remains a known limitation.
The budget vector is operational and does not provide formal differential
privacy. The PL/pgSQL reference runtime has high scale-sensitive
overhead, so a production design should move structural checks and
accounting closer to PostgreSQL plan or executor hooks.
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Canonical functional validation scenarios. The full validation suite passed 24 of 24 scenarios.}
...
\midrule
V01 & Safe view SELECT & \texttt{safe\_view\_select} & \texttt{SELECT} & Allowed & Allowed \\
V02 & Join safe views & \texttt{join\_safe\_views} & \texttt{JOIN} & Allowed & Allowed \\
V03 & CTE & \texttt{cte} & common table expression & Allowed & Allowed \\
V04 & Department totals & \texttt{group\_by} & \texttt{GROUP\ BY} & Allowed & Allowed \\
V05 & Ranked expenses & \texttt{window\_function} & window function & Allowed & Allowed \\
V06 & Scoped drill-down & \texttt{scoped\_drill\_down} & scoped predicate over safe view & Allowed & Allowed \\
V07 & Salary access & \texttt{salary\_access} & denied column & Denied & Denied \\
V08 & Bank account access & \texttt{bank\_account\_access} & denied column & Denied & Denied \\
V09 & Raw table access & \texttt{raw\_table\_access} & schema escape & Denied & Denied \\
V10 & Mutation SQL & \texttt{mutation\_sql} & \texttt{DELETE} & Denied & Denied \\
V11 & DDL & \texttt{ddl} & \texttt{DROP} & Denied & Denied \\
V12 & \texttt{pg\_catalog} access & \texttt{pg\_catalog\_access} & catalog escape & Denied & Denied \\
V13 & \texttt{json\_agg(e)} payload & \texttt{json\_agg\_payload} & aggregation payload & Denied & Denied \\
V14 & \texttt{jsonb\_agg(e)} payload & \texttt{jsonb\_agg\_payload} & aggregation payload & Denied & Denied \\
V15 & \texttt{array\_agg(...)} payload & \texttt{array\_agg\_payload} & aggregation payload & Denied & Denied \\
V16 & \texttt{string\_agg(...)} payload & \texttt{string\_agg\_payload} & aggregation payload & Denied & Denied \\
V17 & \texttt{xmlagg(...)} payload & \texttt{xmlagg\_payload} & aggregation payload & Denied & Denied \\
V18 & \texttt{row\_to\_json(e)} payload & \texttt{row\_to\_json\_payload} & aggregation payload & Denied & Denied \\
V19 & \texttt{json\_build(...)} payload & \texttt{json\_build\_object\_payload} & aggregation payload & Denied & Denied \\
V20 & \texttt{jsonb\_build(...)} payload & \texttt{jsonb\_build\_object\_payload} & aggregation payload & Denied & Denied \\
V21 & Other month & \texttt{out\_of\_scope\_month} & scope escape & Transparent scope filtering & Transparent scope filtering \\
V22 & Other department & \texttt{out\_of\_scope\_department} & scope escape & Transparent scope filtering & Transparent scope filtering \\
V23 & Query budget overflow & \texttt{query\_budget\_overflow} & excessive queries & Denied & Denied \\
V24 & Disclosure budget overflow & \texttt{disclosure\_budget\_overflow} & too many unique expense rows & Denied & Denied \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
PostgreSQL RLS / Oracle VPD & Row predicates enforced near data &
Excellent for standing row policies, but does not make a business task
the runtime object; does not by itself bind approval, TTL, safe-view
versioning, denied fields, SQL-shape policy, cumulative budgets, and
receipts to one exploratory session & Uses database-side enforcement but
wraps it in a task-token and session contract \\
ABAC / XACML / purpose policy & Expressive subject, object, action,
environment, and purpose rules & Requires policy authors to encode
standing rules; does not directly convert one approved analytical task
into a temporary SQL workspace with per-session exposure accounting &
Compiles approved task attributes into deterministic runtime state \\
Short-lived / JIT credentials & Time-bounded authentication and reduced
credential lifetime & Once issued, the credential can exercise whatever
database grants it has; expiry alone does not constrain query shape,
safe views, denied fields, or cumulative disclosure & Credentials are
necessary but must be bound to the task token and checked at execution \\
Audit logs / query auditing & After-the-fact visibility into executed
queries or disclosure risk & Logs alone do not prevent release before an
unsafe query executes; generic audit records do not define budget impact
or prove that each attempt stayed inside a delegated task boundary &
Emits decision receipts and accounts for budget before releasing results \\
Capabilities / OAuth / authenticated delegation & Scoped authority and
delegation from user to agent & Usually scoped to API operations,
resources, or delegated identity; does not resolve arbitrary SQL against
a safe-view registry or account for multi-query database disclosure &
Uses a task token as a database-execution capability \\
PAuth-style task-operation authorization & Checks whether concrete tool
operations are implied by a natural-language task & Focuses on
operation-level authorization; it is not designed to let an agent freely
compose joins, CTEs, aggregates, and windows inside a structured database
boundary & Enforces a structured SQL boundary after approval \\
Data Product MCP / data catalogs & Governed discovery and access to
enterprise data products & Data-product access is coarser than an
approved task run; it does not define one cumulative budget and receipt
chain across exploratory SQL attempts & Turns governed data surfaces
into bounded task sessions \\
Zanzibar-style relationship authorization & Globally consistent
relationship checks over subjects and objects & Answers whether a
subject has a relationship-derived permission; does not answer how an
agent's temporary SQL session should be budgeted, scoped, and audited &
Can feed eligibility checks before issuing a task token \\
\bottomrule
\end{tabular}
\end{table*}

\hypertarget{abac-xacml-capabilities-and-purpose-based-access-control}{%
\subsection{ABAC, XACML, capabilities, and purpose-based access
control}\label{abac-xacml-capabilities-and-purpose-based-access-control}}

Attribute-based access control and XACML provide mature ways to express
authorization policies over subject, object, action, and environmental
attributes~\cite{nist2019abac,oasis2013xacml}. Capability systems
represent authority as an unforgeable object or reference rather than as
ambient identity~\cite{dennis1966programming,miller2003capability}.
Purpose-aware database privacy systems, including Hippocratic databases
and purpose-based relational access control, connect data use to stated
purpose and policy
metadata~\cite{agrawal2002hippocratic,byun2008purpose}.

SessionBound is related to all three lines, but it uses a different
operational object. The unit of approval is an enterprise task, not a
standing role, a general policy rule, or a data subject's privacy
preference. The task token behaves partly like a capability, but it is
bound to a short-lived database credential, safe-view registry, budget
vector, and receipt chain.
...
\hypertarget{prompt-injection-and-agent-security}{%
\subsection{Prompt injection and agent
security}\label{prompt-injection-and-agent-security}}

Prompt injection work shows that LLM-integrated applications blur the
line between data and instructions. Indirect prompt injection can make
retrieved content influence tool calls and data disclosure, and prompt
injection has become a recognized top risk for LLM
applications~\cite{greshake2023indirect,liu2023promptinjection,owasp2025llm01}.
Recent work also explores information-flow control for AI agents as a
way to give stronger guarantees about how labeled information affects
actions~\cite{myers1997difc,costa2025ifcagents}.

SessionBound addresses this risk by removing the LLM from the final
database authorization decision. Prompt injection may cause an agent to
try unsafe SQL, but the database session is still bound to safe views,
...
\hypertarget{database-policy-mechanisms-query-structure-and-auditing}{%
\subsection{Database policy mechanisms, query structure, and
auditing}\label{database-policy-mechanisms-query-structure-and-auditing}}

Database-native mechanisms such as PostgreSQL row-level security and
Oracle Virtual Private Database attach policy enforcement to data
access~\cite{postgresql2026rowsecurity,oracle2026vpd}. SQL injection
defenses such as SQLrand and SQLCHECK show the value of query-structure
analysis and parser-mediated validation for command
integrity~\cite{boyd2004sqlrand,su2006essence}. Query auditing studies
when sequences of database answers disclose protected
information~\cite{kleinberg2000auditing,nabar2006robustness}. DLP and
data leakage prevention systems focus on detecting or preventing
unauthorized sensitive-data movement across endpoints, networks, and
storage~\cite{alneyadi2016dlps,nist2026dlpglossary}.

SessionBound composes these ideas for agent-generated SQL. It uses
database-side safe views and grants for the data-plane boundary,
API-layer AST preflight plus an experimental PostgreSQL hook path for
query-shape checks in the hardening prototype, and receipts plus budgets
for task-session accountability. Unlike generic DLP, SessionBound
accounts for authority spent inside an approved database task session
rather than inspecting all enterprise data flows.
...
auditing systems track when prior answers would reveal protected facts,
while DLP systems focus on sensitive-data movement across enterprise
channels.
...
First, cross-database and lakehouse federation would extend
...
Second, production-grade SQL enforcement should harden the current
database hook prototype into planner- or executor-level enforcement
inside PostgreSQL. The hardening prototype already includes
database-resident structural validation through a
\texttt{post\_parse\_analyze\_hook}; future work should make this path
always on for task sessions, add deeper aggregate-function and
catalog-access policies, and handle generated SQL equivalence more
robustly.

A concrete PostgreSQL production path is to bind the task token once at
session start, keep approved safe-view OIDs in trusted backend state,
inspect the resolved query tree after name resolution, and then attach
task identifiers to the plan or executor state. Planner or executor hooks
can account for returned business-object identifiers and emit receipts
without routing every query through a PL/pgSQL wrapper. This design is the
natural next step suggested by the 100k-row scale sweep.

Third, stronger disclosure accounting is needed. The current
...
Fourth, verifiable receipts can strengthen the audit layer. The
...
Fifth, safe-view authoring and verification should become a
...
\item
  view drift invalidation is implemented through registry-version,
  policy-version, and view-definition hash checks, but production
  deployments still need operational migration and re-approval workflows;
\item
  strict credential-id binding, actor matching, audience validation,
  token replay rejection across a second credential, and same-session
  rebind rejection are implemented in the measured prototype, but require
  production-grade credential brokerage, key management, and audit
  retention outside this demo;
\item
