...
unoptimized: wrapper p50 latency is 78.5--94.4 ms at 1k rows and
6.14--6.31 s at 100k rows, while native hook/executor accounting is
80.0--98.4 ms at 1k rows and 6.24--6.44 s at 100k rows. A separate
...
\(\Delta\) be the nonnegative budget charge for released rows and
business entities. For any allowed query,
...
tuple is checked and debited before that tuple is forwarded; if adding a
tuple would exceed the budget, the runtime emits a denial and errors
before forwarding that tuple. Thus no tuple is intentionally released
without the corresponding nonnegative debit, but native streaming is not
an all-or-nothing guarantee for earlier tuples in a query that later
exceeds budget.
...
materialization or tuple-forwarding boundary.
...
\texttt{native\_end\_to\_end\_benchmark.py}. Exact command lines and
...
\texttt{sessionbound\_guard\_hook\_20260708\_205325.json},
\texttt{hook\_microbenchmark\_20260708\_205831.json},
\texttt{overhead\_breakdown\_20260708\_205527.json}, and
\texttt{native\_end\_to\_end\_20260708\_235456.json/csv}. Scripts write
...
\node[box, below=of h] (b) {Raw / safe-view / RLS at 100k: p50 range 12.3--101.3 ms\\query execution and RLS/audit, no SessionBound budget};
\node[box, below=of b] (w) {Wrapper full SessionBound at 100k: 6.14--6.31 s p50\\PL/pgSQL materialization plus accounting};
\node[box, below=of w] (n) {Native hook/executor accounting at 100k: 6.24--6.44 s p50\\measured but still unoptimized in this prototype};
\end{tikzpicture}
...
RLS+safe-view+audit baselines remain within roughly 110 ms p99 at 100k
rows across these query shapes. Both current SessionBound accounting
paths are much slower: wrapper p50 is 6.14--6.31 s and native
hook/executor p50 is 6.24--6.44 s at 100k rows. Thus the hook-only
...
Each target used 2 warmup and 10 measured iterations per mode and
pattern. API token issuance is excluded from measured query latency.}
\label{tab:scale-sweep}
\begin{tabular}{@{}llrrrc@{}}
\toprule
Rows & Mode & p50 range & max p95 & max p99 & Err. \\
\midrule
1k & Raw & 0.52--1.06 & 1.17 & 1.17 & 0 \\
1k & Safe view & 0.65--1.41 & 1.59 & 1.59 & 0 \\
1k & RLS+SV+Audit & 1.55--1.96 & 2.55 & 2.55 & 0 \\
1k & SB wrapper & 78.48--94.38 & 100.13 & 100.13 & 0 \\
1k & SB native & 80.01--98.43 & 107.87 & 107.87 & 0 \\
10k & Raw & 1.90--8.14 & 8.57 & 8.57 & 0 \\
10k & Safe view & 2.73--9.01 & 9.59 & 9.59 & 0 \\
10k & RLS+SV+Audit & 3.62--10.39 & 11.10 & 11.10 & 0 \\
10k & SB wrapper & 673.83--701.69 & 768.59 & 768.59 & 0 \\
10k & SB native & 660.17--683.23 & 761.76 & 761.76 & 0 \\
100k & Raw & 12.30--94.56 & 99.80 & 99.80 & 0 \\
100k & Safe view & 24.91--101.26 & 110.32 & 110.32 & 0 \\
100k & RLS+SV+Audit & 17.64--100.57 & 103.86 & 103.86 & 0 \\
100k & SB wrapper & 6136.37--6313.42 & 6544.00 & 6544.00 & 0 \\
100k & SB native & 6242.66--6444.11 & 6717.46 & 6717.46 & 0 \\
\bottomrule
\end{tabular}
\end{table}
