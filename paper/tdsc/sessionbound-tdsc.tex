...
release-barrier budget/receipt policy. The native path also covers explicitly
enumerated utility forms such as \texttt{COPY (SELECT)} and \texttt{EXPLAIN}.
...
counters. Both paths stage the candidate result, check aggregate-shape and
tuple-budget policy, and then execute one fenced release-barrier transition
that debits query count, debits released output tuples, and appends the
decision receipt before release. Both paths subtract only nonnegative charges
and record the resulting state in an allow receipt. Therefore the remaining
budget cannot increase across an allowed transition; an over-budget transition
reaches a denial state before any candidate tuple is released.
...
\midrule
\texttt{query\_count} & Wrapper and native paths: one debit inside the fenced release-barrier transition for an allowed execution. Covered preflight and policy denials emit denial receipts without result/entity debit in the current prototype. \\
...
Each receipt records an execution id, task id, actor, query hash or
...
\texttt{receipt\_hash}, where the current hash commits to the execution
id, task id, budget account, binding id, fence token, query hash,
...
acknowledgment. API preflight,
wrapper, hook, controlled-command, and release-barrier denials use the
same audit channel for rollback-surviving denial receipts when the
SessionBound runtime can associate the denial with a bound task. Native
...
query-shape cases, and the PostgreSQL hook evaluation passed 19 of 19
...
The 2026-07-19 database-hook suite evaluated the
\texttt{sessionbound\_guard} path directly and passed 19 of 19
cases in \texttt{sessionbound\_guard\_hook\_20260719\_230317.json}. It
confirmed GUC tamper resistance, allowed bound native
\texttt{SELECT}, support for prepared statements, cursor/\texttt{FETCH},
\texttt{COPY (SELECT)}, and non-\texttt{ANALYZE} \texttt{EXPLAIN}, and
blocked set operations, catalog/raw access, runtime-helper abuse, payload
aggregation, unapproved OIDs, \texttt{HAVING}/direct-entity aggregate
shapes, direct aggregate/window release, holdable cursors, and
\texttt{EXPLAIN ANALYZE}. The companion
\texttt{prebind\_runtime\_20260719\_150119.json} probe passed 7 of 7
...
\texttt{path\_consistency\_20260719\_215758.json}, passed 13 of 13
cases across API, direct-wrapper, and direct-native paths. It compares
39 path observations for allowed detail projection, join, non-aggregate
CTE, denied-column, raw-schema, catalog, set-operation,
aggregate/window, side-effect-function, \texttt{TABLESAMPLE}, and
...
\path{receipt_fault_20260719_135735.json} passed 7 of 7 cases. It
...
allow/deny receipt insertion. The current receipt claim is therefore
