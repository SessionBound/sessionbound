...
The prototype is implemented over PostgreSQL with an accounting-complete
wrapper reference path and a native hook/executor hardening path.
Evaluation validates 24 of 24 canonical scenarios and a 140-case
adversarial SQL suite: 126 tested violations are blocked, 14 safe-view
analytical cases are allowed and accounted for, and all tested direct
small-group aggregate-release attempts are blocked by the minimum-group
policy. In the wrapper-based reference path, full SessionBound has p50
latency of 17.4--18.6 ms across representative query patterns on the
default seed. A diagnostic native end-to-end scale benchmark over
1k/10k/100k scoped rows shows that both accounting-complete paths remain
interactive at 1k rows (75.6--95.5 ms p50) and sub-second around 10k
rows (632.4--647.0 ms p50), but become multi-second at 100k rows
(6.11--6.30 s p50), exposing an unoptimized result-accounting
bottleneck for large detail releases. A separate
hook-only parse/analyze guard microbenchmark shows 0.125--0.138 ms p50
structural-check latency, indicating that structural guarding is not the
dominant bottleneck.
...
the approved boundary. The artifact repository, including code,
synthetic data, validation reports, benchmark outputs, and LaTeX source,
will be released with the submission artifact.
\end{abstract}
...
enforced as API shape checks, wrapper/runtime group-cardinality checks
before row release, and conservative native shape checks for direct SQL.
...
The latest revision run used Docker Compose with PostgreSQL 16.14, the
...
\texttt{high-standard-tdsc-pdsc-revision}. The default seed
...
The latest raw files cited in the paper are
\texttt{ast\_validation\_20260708\_205921.json},
\texttt{adversarial\_sql\_20260708\_235742.json},
\texttt{sessionbound\_guard\_hook\_20260709\_015110.json},
\texttt{hook\_microbenchmark\_20260708\_205831.json},
\texttt{overhead\_breakdown\_20260708\_205527.json},
\texttt{native\_end\_to\_end\_20260709\_013634.json/csv}, and
\texttt{native\_partial\_budget\_20260709\_014913.json}. Scripts write
timestamps, git commit, iteration counts, error counts, and measured
latencies into the raw result files; the tables below are derived from
those files rather than hand-entered experimental claims.
...
The latest revision run uses an adversarial SQL suite with 140 cases.
...
These results expose a scale-sensitive limitation shared by the current
wrapper and native accounting implementations. They should be read as a
diagnostic prototype result, not a production-throughput claim.
...
\scriptsize
\setlength{\tabcolsep}{2.5pt}
\caption{Security-property comparison across evaluated configurations. The RLS baseline combines RLS policies, field-limited safe views, a short-lived read-only role, and basic audit logging, but does not include SessionBound task-token semantics. Credential-token binding includes credential-id matching, actor matching, audience validation, cross-credential replay rejection, expiration, revocation, and same-session rebind rejection. Schema drift invalidation checks safe-view registry version, policy version, view-definition hash, and exposed-column hash at bind time.}
...
\caption{Overhead breakdown p50 latency in milliseconds. The run used 10 warmup and 100 measured iterations per pattern and mode. RLS+SV+Audit is the RLS+safe-view+short-credential+audit baseline.}
...
\caption{Diagnostic native end-to-end scale benchmark in milliseconds.
The p50 column gives the range across SELECT/JOIN/GROUP BY/CTE/window
queries; p95 and p99 are the maximum observed across those patterns.
The 1k and 10k targets use 10 warmup and 100 measured iterations per
mode and pattern; the 100k target uses 10 warmup and 30 measured
iterations. SessionBound modes are split across fresh task sessions when
needed to respect the prototype 100-query task limit. API token issuance
is excluded from measured query latency.}
...
formal SQL safety proof. Its main limitations are:

\begin{itemize}
\tightlist
\item
  the prototype targets a single PostgreSQL database and demo signing
  should be replaced with KMS/JWKS-backed signing and production
...
\item
  minimum-group policy mitigates direct small-group aggregate release,
  but arbitrary semantic inference across multiple allowed answers
  remains out of scope;
\item
  high-risk payload aggregation functions are conservatively blocked,
  and safe views require production migration, review, and re-approval
  workflows;
\item
  current performance results include microbenchmarks, overhead
  ablations, and a 1k/10k/100k native end-to-end diagnostic scale
  benchmark, but not an optimized or broad production workload study;
\item
  evaluated native paths persist allowed receipts and hook/API denial
  receipts through a same-database autonomous audit channel; external
  audit retention and WORM anchoring remain production infrastructure
  outside this demo;
\item
  cross-database federation, malicious DBAs, and compromised hosts are
  out of scope. Future work also includes richer aggregate leakage
  controls, multi-query reconstruction-risk checks, optional DP, and
  planner/executor optimization.
\end{itemize}
...
sessions for AI agents. It is complementary to OAuth, delegation, PAuth,
Data Product MCP, Oracle DDS, Zanzibar, RLS/VPD, and differential
privacy: those mechanisms provide important pieces, while SessionBound
...
sub-millisecond, but both current accounting-complete paths become
multi-second at 100k rows. The production path is therefore not to
weaken the task-bound contract, but to optimize result accounting,
backend state caching, and PostgreSQL executor integration. The
contribution is the task-bound session contract, not a perfect SQL
safety proof.
