...
| Data-bearing database errors | Native executor failures and wrapper dynamic execution failures return fixed agent-facing errors and fixed receipt reasons; query-budget preflight is read-only, while post-execution denials debit query count and emit the denied receipt in one fenced transaction so crashes cannot leave query-count-only orphan transitions. |
