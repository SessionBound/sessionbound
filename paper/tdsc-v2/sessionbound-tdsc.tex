...
\author{Minmin~Wu%
\thanks{Minmin Wu is with China Telecom Global Limited. E-mail:
wuminmin@futurenetech.com.}}
...
\par\noindent Preprint. An arXiv version is available at
...
signatures, token expiration, and revoked task state, but strict
credential-id-to-token matching, actor matching, audience validation,
token replay rejection, and same-session rebind rejection remain
implementation gaps; Section~\ref{evaluation} reports those tests
explicitly.
...
\hypertarget{baseline-scope}{%
\subsection{Baseline scope}\label{baseline-scope}}
...
We compare raw PostgreSQL execution, role-only grants, safe-view-only
access, RLS-only policies, and the full SessionBound runtime path for
core query patterns.
...
iterations per query pattern. The raw baseline is an admin/test role
executing equivalent SQL over \texttt{app\_data} tables with tenant and
month predicates matching the task scope. Role-only, safe-view-only, and
RLS-only baselines use equivalent query patterns under their respective
database controls. The SessionBound path binds a signed task token and
executes \texttt{taskbound.run(sql)} in PostgreSQL. The benchmark
intentionally excludes HTTP latency and dynamic credential creation so
that it focuses on database runtime overhead.
...
0.228/24.535 ms; JOIN 0.223/25.084 ms; GROUP BY 0.249/28.539 ms; CTE
0.215/23.247 ms; and Window 0.269/26.652 ms.

Raw PostgreSQL, role-only, safe-view-only, and RLS-only baselines remain
sub-millisecond at p50 on this small dataset. Full SessionBound adds
larger absolute overhead: 20.9--22.2 ms p50 across the five measured
patterns. Relative overhead is high because the raw PostgreSQL baseline
is extremely small. The likely sources are PL/pgSQL runtime dispatch,
SQL text checks, task-session lookup, safe-view evaluation, query-budget
updates, unique-row exposure accounting, payload-aggregation checks, and
receipt insertion.

We also ran a receipt/budget ablation over three SessionBound query
patterns. The full runtime measured 29.0--30.9 ms p50. Disabling receipts
removed receipt rows as expected, and disabling budget accounting left
\texttt{query\_count} unchanged, but the small dataset produced noisy
latency deltas rather than a stable single-component attribution.

Finally, we ran a synthetic scale sweep over 1k, 10k, and 100k scoped
...
\hypertarget{credential-token-edge-cases}{%
\subsection{Credential-token edge cases and
concurrency}\label{credential-token-edge-cases}}
...
\centering
\scriptsize
\caption{Performance microbenchmark p50 latency in milliseconds. SB denotes SessionBound. Percentages report SB overhead relative to raw PostgreSQL for the same query pattern.}
\label{tab:performance}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
Pattern & Raw & Role & Safe view & RLS & SB & SB overhead \\
\midrule
SELECT & 0.103 & 0.105 & 0.203 & 0.155 & 21.356 & 20700.5\% \\
JOIN & 0.103 & 0.096 & 0.190 & 0.164 & 22.176 & 21344.7\% \\
GROUP BY & 0.108 & 0.118 & 0.201 & 0.187 & 21.152 & 19450.3\% \\
CTE & 0.102 & 0.095 & 0.181 & 0.146 & 20.857 & 20423.8\% \\
Window & 0.148 & 0.194 & 0.253 & 0.217 & 21.271 & 14296.0\% \\
\bottomrule
\end{tabular}
\end{table*}
...
\item
  current performance results include microbenchmarks and a 1k/10k/100k
  synthetic scale sweep, but should not be generalized to production
  workloads or an optimized parser-hook implementation;
\item
