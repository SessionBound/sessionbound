...
%
% TDSC v2 draft derived from paper/tdsc-v1.
...
attacks. The evaluation focuses on functional boundary validation and
database-runtime overhead. It does not yet include role-only,
row-level-security-only, safe-view-only, or no-receipt baselines; those
comparisons remain future evaluation targets.
...
The prototype reports a raw PostgreSQL versus SessionBound
microbenchmark. A broader baseline study would separate access-surface
reduction from task-bound session enforcement across at least five
configurations:
...
The current implementation includes the raw PostgreSQL and full
SessionBound measurements. Role-only, RLS-only, and safe-view-only
measurements remain future work and should use the same task templates,
synthetic datasets, SQL scenario suite, and benchmark environment.

\hypertarget{performance-microbenchmark}{%
\subsection{Performance
microbenchmark}\label{performance-microbenchmark}}

We compare raw PostgreSQL execution against the SessionBound runtime
path for core query patterns.
