...
credential-id-to-token matching and same-session rebind rejection remain
...
strict credential-id matching, token replay rejection across a second
credential, and same-session rebind rejection were not enforced in the
measured prototype.
...
revoked task state were denied. Three intended production properties
were not enforced by the measured prototype: using credential A with
token B was allowed, replaying a token with a second credential was
allowed, and binding a second active task in the same session was
allowed. These results make strict credential-id binding and rebind
rejection implementation requirements rather than demonstrated
...
\caption{Security-property comparison across measured baselines. Partial credential-token binding means that expiration and revocation are enforced, but strict credential-id matching, token replay rejection, and same-session rebind rejection are not yet implemented.}
...
Credential leakage & short-lived DB credential plus required task-token binding; strict credential-id binding remains a prototype gap \\
...
\item
  strict credential-id binding, token replay rejection across a second
  credential, and same-session rebind rejection are not implemented in
