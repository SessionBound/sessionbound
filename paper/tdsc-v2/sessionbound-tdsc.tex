...
properties. The ``Partial'' credential-token result for SessionBound
means that token expiration and revoked task state are enforced, but
strict credential-id matching, actor matching, audience validation, token
replay rejection across a second credential, and same-session rebind
rejection were not enforced in the measured prototype.
...
receipt insertion. These microbenchmark results should not be
generalized to larger datasets without additional experiments.
...
We separately tested credential-token edge cases. Expired tokens and
revoked task state were denied. Five intended production properties were
not enforced by the measured prototype: using credential A with token B
was allowed, replaying a token with a second credential was allowed,
binding a second active task in the same session was allowed, a wrong
token audience was allowed, and a wrong token actor was allowed. These
results make strict credential-id binding, actor/audience validation,
token replay rejection, and rebind rejection implementation requirements
rather than demonstrated properties of the prototype.
...
\caption{Security-property comparison across measured baselines. Partial credential-token binding means that expiration and revocation are enforced, but strict credential-id matching, actor matching, audience validation, token replay rejection, and same-session rebind rejection are not yet implemented.}
...
Credential leakage & short-lived DB credential plus required task-token binding; strict credential-id, actor, audience, replay, and rebind checks remain prototype gaps \\
Audit gap & task receipts \\
Schema drift & safe view version and definition hash checks are required but not implemented in the measured prototype \\
\bottomrule
\end{tabular}
\end{table}
...
\item
  view drift invalidation is a design requirement and is not implemented
  in the measured prototype;
\item
  ...
  rebind rejection are not implemented in the measured prototype;
\item
