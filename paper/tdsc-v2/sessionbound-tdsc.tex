...
\texttt{taskbound.bind\_task}; I3--I4 are checked by the AST preflight
and database permissions; I5 is enforced by safe-view predicates; I6 is
implemented through task execution state; and I7 is implemented through
allow/deny receipts. Production implementations should move the
structural checks into parser-, planner-, or extension-level database
hooks, but the invariant set is independent of that implementation
choice.
...
The current prototype uses AST-level API preflight, conservative
database-side checks, and PostgreSQL permissions. Agents do not receive
raw schema grants. Agents call \texttt{taskbound.run(sql)}. The runtime
role cannot access \texttt{app\_data} directly. Safe views are the query
surface. High-value writes go through controlled commands.

The AST preflight is intentionally framed as a prototype: it runs after
token binding and before invoking \texttt{taskbound.run}. If preflight
denies a query, the API records a denial receipt through
\texttt{taskbound.fail\_receipt} and does not call the runtime wrapper.
The database still enforces no raw schema grants, safe views, budgets,
and receipts. This is stronger than purely lexical checks, but it is not
a PostgreSQL parser hook.

A production implementation should move the same structural policy into:
...
aggregation functions. The AST validation evaluation passed 17 of 17
query-shape cases.
...
The v3 hardening run used Docker Compose with PostgreSQL 16.14 and the
FastAPI SessionBoundDB prototype. The repository branch was
\texttt{tdsc-v3-hardening}, commit \texttt{c95e6ed}. The default seed
contains 430 expenses, 240 employees, and 124 departments. The v3 raw
outputs are stored under \texttt{paper/tdsc-v3/raw\_results/}.
...
overhead breakdown uses direct PostgreSQL connections from the Compose
network so that it isolates database runtime overhead from HTTP, model
calls, dynamic credential creation, and API-layer AST parsing.
...
The v3 hardening run adds an adversarial SQL suite with 28 cases across
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Adversarial SQL evaluation summary from the v3 hardening suite.}
...
does show that the v3 prototype is no longer limited to literal string
...
The v3 overhead breakdown measures five query patterns: simple
...
The RLS-only row is marked \texttt{N/A} in this v3 run because the
current prototype does not define PostgreSQL RLS policies. Earlier v2
experiments separately measured an RLS baseline, but the v3 breakdown
...
safe-view-only p50 is 13.5--17.5 ms. Full SessionBound p50 is
14.7--18.9 ms. Disabling receipts or budget accounting does not
...
The v3 hardening results improve direct SQL-structure coverage and make
the overhead source clearer, but several gaps remain. The AST validator
is still an API-layer preflight rather than a PostgreSQL parser hook.
Small-group aggregate inference remains a known limitation. The budget
vector is operational and does not provide formal differential privacy.
The PL/pgSQL reference runtime has high scale-sensitive overhead, so a
production design should move structural checks and accounting closer to
PostgreSQL parse, plan, or executor hooks.
...
\midrule
SELECT & 0.222 & 0.242 & 14.316 & N/A & 15.085 & 14.554 & 14.672 \\
JOIN & 0.167 & 0.171 & 17.474 & N/A & 18.445 & 18.482 & 18.859 \\
GROUP BY & 0.174 & 0.208 & 13.942 & N/A & 17.335 & 15.448 & 15.307 \\
CTE & 0.173 & 0.212 & 13.529 & N/A & 15.264 & 14.871 & 15.905 \\
Window & 0.416 & 0.405 & 15.698 & N/A & 18.348 & 18.188 & 17.380 \\
\bottomrule
\end{tabular}
\end{table*}
...
AST-level preflight for query-shape checks in the prototype, and
receipts plus budgets for task-session accountability. Unlike generic
DLP, SessionBound accounts for authority spent inside an approved
database task session rather than inspecting all enterprise data flows.
...
Second, production-grade SQL enforcement should move from
API-layer AST preflight, conservative checks, and database permissions
toward parser-, planner-, or executor-level enforcement inside the
database. This includes database-resident structural validation,
PostgreSQL hooks, aggregate-function policies, catalog-access controls,
and robust handling of generated SQL equivalence.

A concrete PostgreSQL extension path is to bind the task token once at
session start, then use parse/analyze hooks to inspect the resolved query
tree rather than SQL text. The hook can reject relations outside the
safe-view registry, columns outside the task capability, mutating
commands, catalog access, and disallowed aggregate functions after name
resolution. Planner or executor hooks can attach task identifiers to the
plan, account for returned business-object identifiers, and emit receipts
without routing every query through a PL/pgSQL wrapper. This design is
the natural next step suggested by the 100k-row scale sweep.
...
\tightlist
\item
  SQL validation now includes AST-level API preflight, but it is not
  production-grade PostgreSQL parser/planner-hook enforcement;
\item
...
\item
  current performance results include microbenchmarks, v3 overhead
  ablations, and a prior 1k/10k/100k synthetic scale sweep, but should
  not be generalized to production workloads or an optimized parser-hook
  implementation;
\item
